...
// Custom debounce function to delay search input updates
...
const AdminPanel = () => {
  // State for managing event selection, data, and UI controls
  ...
  const [limit, setLimit] = useState(20); // Number of records per page
  ...
  // Function to fetch registrations and members from the API
  const fetchAllData = async () => {
    ...
    try {
      ...
      setTotalRecords(regData.length); // Set total records for pagination
  ...
  // Process registrations with lazy-loaded member data
  const processRegistrations = useMemo(() => {
    ...
    registrations.forEach((reg) => {
      ...
      const teamLeader = {
        ...
        name: reg.name || leaderInfo.name || fields["name"] || "Unknown",
        email: reg.email || leaderInfo.email || fields["email"] || "N/A",
  ...
  // Apply client-side pagination to filtered registrations
  ...
  }, [allParticipants, selectedEvent, searchTerm, sortField, sortOrder, page, limit]);

  // Calculate team counts per event
  const eventTeamCounts = useMemo(() => {
    return allParticipants.reduce((acc, reg) => {
      const eventId = reg.eventId;
  ...
  // Sort event team counts by count (descending)
  ...
  // Toggle row expansion for lazy-loaded details
  ...
  // Handle image modal for payment receipts
      ...
      {event.name}
          ...
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="Search"
              onChange={(e) => handleSearchChange(e.target.value)} // Use debounced search
        ...
        <div className="mb-6 flex flex-col sm:flex-row gap-4 items-center justify-center">
          ...
          <select
            ...
            onChange={(e) => {
              ...
              setPage(1); // Reset to first page on sort order change
            }}
            className="w-full sm:w-48 p-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 bg-white"
          >
                    ...
                    {filteredRegistrations.map((reg, index) => (
                      <React.Fragment key={index}>
                        <tr
                          ...
                          } border-b hover:bg-indigo-50 transition duration-200`}
                        >
                            ...
                            <td colSpan="9" className="p-4">
                              <div className="border border-gray-300 rounded-lg bg-white p-4 shadow-sm">
                                ...
                                <div className="mb-4">
                                  ...
                                  <div className="text-sm text-gray-700 space-y-1">
                                    <p>
                                      <span className="font-medium">Name:</span> {reg.name || "N/A"}
                                    </p>
                                    <p>
                                      <span className="font-medium">Email:</span> {reg.email || "N/A"}
                                    </p>
                                    <p>
                                      <span className="font-medium">College:</span> {reg.college || "N/A"}
                                    </p>
                                    <p>
                                      <span className="font-medium">Phone:</span> {reg.phone || "N/A"}
                                    </p>
                                    <p>
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                                      {reg.teamMembers.map((tm, idx) => (
                                        ...
                                        >
                                          ...
                                          <p className="text-xs text-gray-600">
                                            <span className="font-medium">College:</span> {tm.college}
                                          </p>
                                          <p className="text-xs text-gray-600">
                                            <span className="font-medium">Phone:</span> {tm.phone}
                                          </p>
                                          <p className="text-xs text-gray-600">
                                            <span className="font-medium">Member ID:</span> {tm.memberId}
                                          </p>
                                        </div>
                                      ))}
                                    </div>
        ...
        {/* Image Modal for Payment Receipts */}
        {selectedImage && (
